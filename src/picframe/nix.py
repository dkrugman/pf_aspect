import os, sys, re, time, warnings, sqlite3
import requests
...
def get_single_playlist_media(session, playlist_url, item_path, playlist_id, playlist_name, db=None):
...
def get_playlist_media(session, playlist_url, item_path, playlists_to_update, db=None):
    ...
    for playlist_id, playlist_name, subdirectory in playlists_to_update:
        ...
        slides, nix_lastVersion = get_single_playlist_media(session, playlist_url, item_path, playlist_id, playlist_name, db)
...
# LOGIN
    try:
        session = create_authorized_client(importer.username, importer.password, importer.login_url)
...
# GET PLAYLIST NAMES 
    playlists = []
    try:
        playlists = get_playlist_names(session, importer.playlist_url, importer.frame_key)
...
# CHECK OR CREATE SUBDIRECTORIES
    ...
    try:
        ...
        media_items, last_version = get_playlist_media(session, importer.playlist_url, importer.item_path, playlists_to_update, db)
