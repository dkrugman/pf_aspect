...
DEFAULT_CONFIG = {
    ...
    "model": {
        ...
        "image_attr": [
            "PICFRAME GPS"
        ],  # image attributes send by MQTT, Keys are taken from exifread library, 'PICFRAME GPS' is special to retrieve GPS lon/lat # noqa: E501
        "load_geoloc": True,
        "locale": "en_US.utf8",
        "key_list": [
    ...
    "aspect": {
        ...
        "sources": {
            ...
            "apple_photos": {
                "enable": False,
                "playlist_url": "https://photos.apple.com/api/v1/playlists",
                "acct_name": "user",
                "acct_pwd": "password",
                "aspect_identifier": "OLED",
            },
        ...
        "services": {
            "random_org": {
                ...
                "key1_name": "aspect_dev1",
                "daily_limit": 1000,  # requests per day
                "rate_limit ": 10,  # requests per second
            },
            "croppola": {
                "enable": False,
                "api_url": "https://cropolla.com/api/a",
                "api_id": "id",
                "api_key": "key",
                "max_crop": 0.25,
            },
...
class Model:
    def __init__(self, configfile=DEFAULT_CONFIGFILE):
                ...
                for section in ["viewer", "model", "mqtt", "http", "peripherals", "aspect"]:
                    if section not in conf:
                        ...
                        self.__logger.warning(
                            "Config file %s does not contain section '%s'. Skipping sec.", configfile, section
                        )
                    else:
        ...
        self.__geo_reverse = geo_reverse.GeoReverse(
            model_config["geo_key"], key_list=self.get_model_config()["key_list"]
        )
    ...
    def get_http_config(self):
        if (
            "auth" in self.__config["http"]
            and self.__config["http"]["auth"]
            and self.__config["http"]["password"] is None
        ):
    ...
    def get_number_of_files(self):
        return sum(sum(1 for pic in pics if pic is not None) for pics in self.__file_list)
